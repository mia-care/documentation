...
Use the search bar to find requirements by title or ID. The filter panel lets you narrow by type, status, quality level, and assignee. For large projects, the flat view with filters is the fastest way to work through a backlog.
...
When importing requirements from a CSV file, each row is validated individually. If some rows are valid and others are not, the platform imports the valid rows and reports errors for the rejected ones — you do not need to fix everything before any requirements are created. Error details are shown per row so you can correct and re-import only the failed entries.
